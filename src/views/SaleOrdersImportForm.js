...
const SalesOrdersImportForm = (props) => {
  const [uploadFile, setUploadFile] = useState([]);

  function handleImportSaleOrders(event) {
    event.preventDefault();

    const dataArray = new FormData();
    dataArray.append("file", uploadFile);
    const { onFormHandleSubmit } = props;
  ...
  function handleOnChangeImportFile(file) {
    setUploadFile(file);
  }
  ...
  return (
    <>
      ...
      <form onSubmit={handleImportSaleOrders}>
        <fieldset>
          ...
          <input
            ...
            name="file"
            onChange={(e) =>
              handleOnChangeImportFile(e.target.files[0] ?? null)
            }
            required
          />
          <br />
